...
int main(int argc, char *argv[])
{
	...
	localAddress.sin_port = htons(atoi(argv[3]));
	...
	/* Handshake */
	serverHandshake(socketId, &distantAddress);
	...
	*/ printf("\n");
	while (!fini)
	{
			printf("transmis, recu, accuse : %d %d %d\n", transmis, recu, accuse);
		...
		*/
						...
						tailleSocket)) == -1)
		{
			...
			printf("nbLu - (sizeof(struct message) - BUFFER_LENGTH) : %lu", nbRecu - (sizeof(struct message) - BUFFER_LENGTH));
			perror("sendto");
			exit(1);
		}
